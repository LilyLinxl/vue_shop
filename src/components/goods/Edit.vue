...
<div>
  ...
  <el-card>
      <el-alert type="info" title="修改商品信息" show-icon center></el-alert>
      ...
      :before-leave="beforeTabLeave" @tab-click="tabClicked">
        <el-tab-pane label="基本信息" name="0">
            <el-form-item label="商品名称" prop="goods_name">
              <el-input v-model.trim="editForm.goods_name" placeholder="请输入商品名称"></el-input>
            </el-form-item>
            <el-form-item label="商品价格" prop="goods_price">
              <el-input v-model.trim="editForm.goods_price" type="number"></el-input>
            </el-form-item>
            <el-form-item label="商品重量" prop="goods_weight">
              <el-input v-model.trim="editForm.goods_weight" type="number"></el-input>
            </el-form-item>
            <el-form-item label="商品数量" prop="goods_number">
              <el-input v-model.trim="editForm.goods_number" type="number"></el-input>
            </el-form-item>
            <el-form-item label="商品分类" prop="goods_cat">
              <el-cascader  :options="catelist" :props="cateProps"
              v-model="editForm.goods_cat" @change="handleChange"></el-cascader>
            </el-form-item>
        </el-tab-pane>
        ...
        <el-tab-pane label="商品内容" name="4">
          <!-- 富文本编辑器组件 -->
          <div class="ql-snow">
          <quill-editor v-model="editForm.goods_introduce" class="ql-editor"></quill-editor>
          </div>
...
export default {
  ...
  methods:{
    ...
    beforeTabLeave(activeName,oldActiveName){
      if(oldActiveName === '0'){
        if(this.editForm.goods_cat.length !== 3){
          ...
          return false
        }else if(this.editForm.goods_name===''){
          this.$message.error('请输入商品名称')
          return false
        }else if(this.editForm.goods_price=='0'){
          this.$message.error('请输入商品价格')
          return false
        }else if(this.editForm.goods_weight===''){
    ...
    handleRemove(file){
      ...
      // 查找符合条件的索引
      const index = this.editForm.pics.findIndex(item=>item.pics
      === filePath)
      // 移除索引对应的图片
      this.editForm.pics.splice(index,1)
    },
    handleSuccess(response) {
      // 将服务器返回的临时路径保存到editForm表单的pics数组中
    ...
    edit(){
      this.$refs.editFormRef.validate(async valid => {
        ...
        //将 editForm进行深拷贝，避免goods_cat数组转换字符串之后导致级联选择器报错
